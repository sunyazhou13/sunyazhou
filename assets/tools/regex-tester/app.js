...
(function () {
  ...
  function setWarn(items) {
    ...
    els.warn.innerHTML = '<span class="rt-warn-title">⚠ 回溯风险提示</span><ul>' +
  ...
  function compile() {
    ...
    try {
      ...
      return { ok: false, error: '正则无效：' + (e && e.message || e) };
    }
  }
  ...
  function detectDanger(src) {
    ...
    // 嵌套量词，如 (\w+)+ / (\d*)* / (a+)+b
    if (/\([^()]*[+*][^()]*\)[+*?]/.test(src)) {
      out.push('检测到嵌套量词（如 (\\w+)+、(\\d*)*）：在极端输入下可能引发灾难性回溯，建议改写为原子的、非贪婪或更精确的匹配');
    }
    // 连续通配量词，如 .*. * 链
    if (/(?:\.[*+]){2,}/.test(src)) {
      out.push('检测到连续的 .*、.+ 通配链（如 .*.*）：可能引发过度回溯，建议合并或限制范围');
    }
    // 重叠的可选重复字符组
    if (/(\([^()]*\|[^()]*\)[+*])\1/.test(src)) {
      out.push('检测到重叠的可选分支重复，存在回溯放大风险');
    }
    return out;
  }
  ...
  function buildHighlighted() {
    ...
    state.matches.forEach(function (m, idx) {
      ...
      mark.title = '匹配 ' + (idx + 1) + ' · 位置 ' + m.index + '–' + (m.index + m[0].length);
  ...
  function renderGroups() {
    ...
    var html = '<div class="rt-groups-title">捕获分组</div><table class="rt-gtable"><thead><tr><th>#</th>';
    for (var j = 0; j < maxGroups; j++) {
      html += '<th>' + (j === 0 ? '整体' : '组 ' + j) + '</th>';
    }
    html += '<th>位置</th></tr></thead><tbody>';
    ...
    html += '</tbody></table>';
    if (list.length > shown) html += '<p class="rt-groups-more">仅显示前 ' + shown + ' 个匹配</p>';
    els.groups.innerHTML = html;
  }

  function updateMeta(count, ms, charCount) {
    els.meta.textContent = charCount + ' 个字符 · ' + count + ' 处匹配 · 耗时 ' +
      (ms >= 1 ? Math.round(ms) + ' ms' : '<1 ms');
  }
  ...
  function render() {
    ...
    // 回溯/复杂度监控：超时即可疑
    if (dt > SLOW_MS) {
      danger.push('本次匹配耗时 ' + Math.round(dt) + ' ms，显著偏慢，输入规模或正则设计可能触发回溯放大');
    }
    setWarn(danger);

    if (!matches.length) {
      note(charCount ? '无匹配' : '请输入测试文本', 'warn');
      els.meta.textContent = charCount + ' 个字符 · 0 处匹配 · 耗时 ' +
        (dt >= 1 ? Math.round(dt) + ' ms' : '<1 ms');
  ...
  els.text.value = '联系 user1@example.com 与 admin@test.cn，更多：foo@bar.io\n或 alt+key 组合，再看 123-456-7890 是否命中';
  render();
})();
