...
在学习《Metal》 GPU着色器编程中 有一张讲解GPU资源堆(Resource Heap)的参数缓冲(Arguement Buffer)送资源到GPU时遇到一段代码计算资源的内存占用很有意思,在此记录下来.
...
是的，表达式 `129 & (128 - 1)` 本质上是一种高效的取模（求余）操作，但**仅当除数是 2 的幂次方时成立**。以下是详细解释：

---

### **1. 数学原理**
- **当 `align` 是 2 的幂次方**（如 128 = 2⁷），`align - 1` 的二进制形式是全 `1`（例如 127 的二进制是 `01111111`）。
- **按位与（&）操作**会将 `size` 的高位清零，仅保留低 `log₂(align)` 位的值，即 `size % align` 的余数。
...
#### **限制**
- **仅适用于 2 的幂次方**：若 `align` 不是 2 的幂次方（如 100），此方法失效。
