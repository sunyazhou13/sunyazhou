...
The main idea is to use `CTFramesetterSuggestFrameSizeWithConstraints` to calculate the height and width of the text. Remember to set the related properties (line spacing and automatic line wrapping) for the `CTFramesetterRef`. The following is an Objective-C code example that uses CoreText to draw text and calculate its height:
